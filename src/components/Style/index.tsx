...
const Style: FC<StyleProps>= ({ className, styleProps, devMode, backgroundColour, textColour, children}) => {
        ...
        <div className="container mx-auto w-full">
            ...
            <div className="px-4 sm:px-6 lg:px-8">
                ...
                <div className={cn(
                    ...
                    <div className={cn(
                        styleProps && styleProps.padding?.bottom ? `pb-${remToTailwind(styleProps.padding?.bottom )}` : "",
                        styleProps && styleProps.padding?.top ? `pt-${remToTailwind(styleProps.padding?.top )}` : "",
                        styleProps && styleProps.padding?.left ? `pl-${remToTailwind(styleProps.padding?.left )}` : "",
                        styleProps && styleProps.padding?.right ? `pr-${remToTailwind(styleProps.padding?.right )}` : "",
                        )}>
                        {/* Custom padding */}
                        <div className={cn(
                            "flex flex-col w-full",
                            styleProps?.alignment ? `items-${styleProps.alignment}` : "items-start",
                            styleProps?.justification ? styleProps.justification : "justify-start",
                            styleProps && styleProps?.backgroundColor?.opacity ? opacityToTailwind(styleProps.backgroundColor.opacity) : "",
                            className,
                            devMode ? "bg-blue-300" : "", 
                            )}
                            style={!devMode ? {backgroundColor: styleProps?.backgroundColor?.colour?.hex} : {}}
                            >
